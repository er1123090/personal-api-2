...
def assign_user_utterances(
...
) -> List[Tuple[str, Any]]: # Return type changed to Any to allow list of dicts
                ...
                for slot, value in matches:
                    if slot in ground_truth_pref_slots:
                        ground_truth_objs.append({
                            "domain": domain,
                            "slot": slot,
                            "value": [to_str(value)] # Wrap single value in list
                        })

                if ground_truth_objs:
                    # easy 케이스는 보통 하나의 구체적인 값을 가지므로 리스트로 감싸서 전달
                    ground_truth = {"ground_truth": ground_truth_objs}
                    results.append((query_map[domain], ground_truth))
        return results
    ...
    # [CASE 2] medium
        ...
        # Collect domains used in history to exclude them if necessary (or strictly follow logic)
        # Medium usually involves re-stating preferences for known domains or implicit pref.
        # Based on your previous logic, checking evidence:

        for pref in prefs:
            group_name = pref.get("value_group")
            if group_name in pref_group_data:
                seen_domains = set() # To avoid duplicates per preference group iteration if needed
                for evidence in pref.get("evidence", []):
                    domain = evidence.get("domain")
                    slot = evidence.get("slot")
                    # evidence에 'values' 리스트가 있거나 단일 'value'가 있을 수 있음
                    # 데이터셋 구조에 따라 안전하게 처리
                    vals = []
                    if "values" in evidence and isinstance(evidence["values"], list):
                        vals = [to_str(v.get("value")) for v in evidence["values"] if v.get("value") is not None]
                    elif "value" in evidence:
                        vals = [to_str(evidence["value"])]
    ...
    # [CASE 3] hard
        ...
        for pref in prefs:
            ...
            # Domains already used in the conversation (history)
            used_domains = {e.get("domain") for e in pref.get("evidence", []) if e.get("domain")}
            
            # Retrieve all rules for this group
            rules = pref_group_data[current_group_name].get("rules", [])
            
            # 1. Identify candidate domains first (in rules but NOT in used_domains)
            ...
            # 2. For each candidate domain, aggregate all applicable rules/values
            for cand_domain in candidate_domains:
                # Dictionary to aggregate values by slot: { "slot_name": ["val1", "val2"] }
                slot_values_map = {}
                ...
                # 3. Construct the final list of dicts
                ground_truth_objs = []
                for s, v_list in slot_values_map.items():
                    ground_truth_objs.append({
                        "domain": cand_domain,
                        "slot": s,
                        "value": v_list # This creates the ["1", "2"] format
                    })
                
                if ground_truth_objs:
                    ground_truth = {"ground_truth": ground_truth_objs}
                    results.append((query_map[cand_domain], ground_truth))
