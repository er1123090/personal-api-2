...
# ---------------------------------------------------------
# 4. vLLM API Call Wrapper (UPDATED: Extract Reasoning)
# ---------------------------------------------------------
async def call_vllm_api(
    ...
    try:
        ...
        # 1. Extract Reasoning via <think> tags (regardless of tool use)
        think_match = re.search(r"<think>(.*?)</think>", raw_content, flags=re.DOTALL)
        if think_match:
            result["reasoning"] = think_match.group(1).strip()
        
        # 2. Clean content (remove think tags for the output)
        clean_content = re.sub(r"<think>.*?</think>", "", raw_content, flags=re.DOTALL).strip()

        # 3. Determine Output (Tool Call vs Text)
        if message.tool_calls:
            ...
            # Fallback: Check if function call syntax exists in text
...
# ---------------------------------------------------------
# 5. Async Worker & Pipeline (UPDATED: Save Reasoning)
# ---------------------------------------------------------
async def process_single_item(
    ...
    async with semaphore:
        ...
        # UPDATED: call_vllm_api returns dict now
        llm_result = await call_vllm_api(client, prompt, model_name, tools_schema)
        ...
        log_record = {
            ...
            "model_reasoning": llm_reasoning, # NEW: Save reasoning in log
        }
        ...
        current_ex["llm_reasoning"] = llm_reasoning # NEW: Save reasoning in result
...
if __name__ == "__main__":
    ...
    asyncio.run(process_vllm_pipeline(
        ...
        selected_template, args.prompt_type, args.context_type, 
