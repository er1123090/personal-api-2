...
# ---------------------------------------------------------
def load_multiturn_pool_by_ds(fpath: str) -> Dict[Tuple[str, str], List[Dict]]:
    """
    Index multi-turn query dataset by (domain, slot).
    ...
    print(f"[Info] Loaded multi-turn pool (by ds): {len(data)} queries -> {len(pool)} domain-slot keys.")
    return pool


def load_multiturn_pool_by_domain(fpath: str) -> Dict[str, List[Dict]]:
    """
    Index multi-turn query dataset by domain only.
    Returns: Dict[domain, List[query_objects]]
    """
    if not os.path.exists(fpath):
        print(f"[Error] Multi-turn query file not found: {fpath}")
        return {}
    ...
    print(f"[Info] Loaded multi-turn pool (by domain): {len(data)} queries -> {len(pool)} domain keys.")
    return pool
...
# ---------------------------------------------------------
# Parsing Helpers for API Call Merge
# ---------------------------------------------------------
def _parse_func_call(call_str: str) -> Tuple[str, Dict[str, str]]:
    ...
    NOTE: This parser extracts only quoted values key="val" or key='val'.
    """
...
def build_reference_ground_truth_single_slot(
    base_api_call: str, func_domain: str, slot: str, values: List[str]
) -> List[str]:
    """
...
def build_reference_ground_truth_multi_slot(
    base_api_call: str, func_domain: str, slot_values: Dict[str, str]
) -> List[str]:
    """
    EASY alignment with singleturn:
      - Combine all extracted pref slots from the original api_call into ONE GT call.
      - Return a single-element list [Func(...)].
    """
    base_api_call = (base_api_call or "").strip()
    func_domain = (func_domain or "").strip()
    ...
    merged = dict(base_args)
    for s, v in (slot_values or {}).items():
        if s and v is not None:
            merged[s] = str(v).strip()

    return [_render_func_call(func, merged)]


# ---------------------------------------------------------
# 2. Logic to Assign User Utterance (Multi-turn)
# ---------------------------------------------------------
def assign_user_utterances(
    pref_list_path: str,
    example: Dict[str, Any],
    query_map: Dict[str, str],  # kept for compatibility (not required in multiturn)
    pref_type: str,
    multiturn_pool_by_ds: Dict[Tuple[str, str], List[Dict]],
    multiturn_pool_by_domain: Dict[str, List[Dict]],
    ...
    DESIGN GOAL:
      - easy: match singleturn behavior -> ONE query per domain(call), not per (domain,slot)
      - medium/hard: keep (domain,slot) behavior
    """
    ...
    # -----------------------------------------------------
    # 1) Identify target preferences
    # -----------------------------------------------------
    # easy: domain -> slot_values (single value each)
    domain_to_slotvals: Dict[str, Dict[str, str]] = defaultdict(dict)
    # medium/hard: list of {domain, slot, values(list)}
    ...
    if pref_type == "easy":
        ...
        if isinstance(api_calls, list):
            for call_str in api_calls:
                ...
                pattern = r'(\w+)=["\']([^"\']+)["\']'
                matches = re.findall(pattern, args_content)
                ground_truth_pref_slots = set(pref_list.get(domain, []))

                for slot, value in matches:
                    if slot in ground_truth_pref_slots:
                        # single exact value for easy
                        domain_to_slotvals[domain][slot] = _to_str(value)

        # If nothing found, return empty
        if not domain_to_slotvals:
            return []
        ...
        for pref in prefs:
            ...
            for evidence in pref.get("evidence", []):
                ...
                if cand_vals:
                    target_preferences.append({"domain": d, "slot": s, "values": cand_vals})
                    seen_ds.add((d, s))

    elif pref_type == "hard":
        # [MODIFIED] Hard Logic: Evidence에 없는 Domain만 Target으로 선정
        prefs = example.get("api_calls_pref", [])
        if not isinstance(prefs, list) or not prefs:
            return []
        if not pref_group_data:
        ...
        for pref in prefs:
            ...
            # 1. Evidence에 등장한 Domain 수집
            evidence_domains = set()
            for evidence in pref.get("evidence", []):
                ev_d = evidence.get("domain")
                if ev_d:
                    evidence_domains.add(ev_d)

            # 2. Group 내의 모든 (Domain, Slot) 중 Evidence에 없는 것만 필터링
            ds_values = group_value_map[group_name]
            for (d, s), cand_vals in ds_values.items():
                if d in evidence_domains:
                    # 이미 Evidence로 쓰인 도메인은 Hard(Generalization) 대상이 아님
                    continue
    ...
    # -----------------------------------------------------
    # 2) Build (context, GT) using multi-turn pool
    # -----------------------------------------------------
    def _extract_context_from_mt_query(mt_query: Dict[str, Any]) -> Tuple[Dict[str, Any], str]:
        # Dialogue History
        dialogue_list = mt_query.get("query", [])
        temp_history: List[Tuple[str, str]] = []
        ...
        return input_context, base_api_str

    # ---- EASY: ONE query per domain(call) (singleturn-aligned)
    if pref_type == "easy":
        for domain, slotvals in domain_to_slotvals.items():
            # pick representative multiturn query by domain
            candidates = multiturn_pool_by_domain.get(domain, [])
            if not candidates:
                continue

            mt_query = candidates[0]
            input_context, base_api_str = _extract_context_from_mt_query(mt_query)

            reference_ground_truth = build_reference_ground_truth_multi_slot(
                base_api_call=base_api_str,
                func_domain=domain,
    ...
    # ---- MEDIUM/HARD: keep per-(domain,slot) queries
    for target in target_preferences:
        key = (target["domain"], target["slot"])
        candidates = multiturn_pool_by_ds.get(key)
        if not candidates:
            continue
        ...
        reference_ground_truth = build_reference_ground_truth_single_slot(
            base_api_call=base_api_str,
...
async def call_llm_api_async(
    ...
    try:
        ...
        if "gemini" in model_name.lower():
            ...
            response = await client.aio.models.generate_content(model=model_name, contents=prompt, config=conf)
        ...
        # --- OPENAI LOGIC ---
            ...
            if getattr(message, "tool_calls", None):
                ...
                output = match.group(0).strip() if match else clean_content
...
async def process_single_item(
    ...
    async with semaphore:
        ...
        llm_output = await call_llm_api_async(prompt, model_name, openai_client, tools_schema, reasoning_effort)
...
async def process_with_llm_async(
    ...
    multiturn_pool_by_ds = load_multiturn_pool_by_ds(multiturn_query_path)
    multiturn_pool_by_domain = load_multiturn_pool_by_domain(multiturn_query_path)

    if not multiturn_pool_by_ds and not multiturn_pool_by_domain:
    ...
    skipped_count = 0
    print(f"Starting ASYNC process (Multi-turn)... (Model: {model_name}) | PrefType: {pref_type}")
    ...
    for _, row in df.iterrows():
        ...
        pairs_list = assign_user_utterances(
            ...
            query_map=query_map,
            pref_type=pref_type,
            multiturn_pool_by_ds=multiturn_pool_by_ds,
            multiturn_pool_by_domain=multiturn_pool_by_domain,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="/data/minseo/experiments4/query_multiturn.json",
    ...
    selected_template = IMPLICIT_ZS_PROMPT_TEMPLATE  # keep as-is
